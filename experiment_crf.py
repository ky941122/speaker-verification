...
# @Date   : 2019-07-24
# @Author : KangYu
# @File   : experiment_hierachicalRNN.py
...
def train_attention(config):
    ...
    fw_cell_2 = tf.nn.rnn_cell.GRUCell(config.rnn_units // 2, name="forward_cell_2")
    bw_cell_2 = tf.nn.rnn_cell.GRUCell(config.rnn_units // 2, name="backward_cell_2")
    rnn_out_2, _ = tf.nn.bidirectional_dynamic_rnn(fw_cell_2, bw_cell_2, dense1, sequence_length=seq_len,
                                                 dtype=tf.float32)
    rnn_out_2 = tf.concat(rnn_out_2, axis=-1)

    dense2 = tf.layers.dense(inputs=rnn_out_2, units=64, activation=tf.nn.relu,
                             kernel_initializer=tf.truncated_normal_initializer(),
    ...
    log_likelihood, transition_params = tf.contrib.crf.crf_log_likelihood(logits, label, seq_len)  #[batch]
    ...
    loss = log_likelihood + config.alpha * tf.reduce_mean(attention_loss / tf.cast(tf.reduce_sum(balance_mask, axis=1), dtype=tf.float32))
    ...
    viterbi_sequence, viterbi_score = tf.contrib.crf.crf_decode(logits, transition_params, seq_len)

    pred_label = viterbi_sequence
    ...
    train_root = '/share/kangyu/speaker_verification/new_data/dahai/train_no2'
    ...
    test_zhikang_root = '/share/kangyu/speaker_verification/new_data/zhikang/test_no2'
    ...
    test_dahai_root = '/share/kangyu/speaker_verification/new_data/dahai/test_no2'
