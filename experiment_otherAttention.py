...
# @Date   : 2019-07-24
# @Author : KangYu
# @File   : experiment_otherAttention.py
...
def train_attention(config):
    ...
    #                           kernel_initializer=tf.truncated_normal_initializer(), name='attention_w1')
    # voice_K = tf.layers.dense(inputs=voice_embed, units=32, activation=None, use_bias=False,
    #                           kernel_initializer=tf.truncated_normal_initializer(), name='attention_w2')
    word_V = word_embed  # shape: (batch, max_length, 200)
    voice_A = tf.matmul(voice_embed, voice_embed, transpose_b=True)  # shape: (batch, max_length, max_length)
...
if __name__ == "__main__":
    ...
    config.model_name = "otherAttention2"
