...
def positional_encoding(inputs, maxlen, headskip=0, scope="positional_encoding"):
    '''
    @inputs: tensor and shape: [batch*max_sent_num, max_sent_len, 200]
    @maxlen: must larger than max_sent_len, which will be used for generate the dictionary
    @masking: boolean variable indicate whether need to mask the words
    @headskip: skip the head position
    '''
    E = inputs.get_shape().as_list()[-1] # static
    N, T = tf.shape(inputs)[0], tf.shape(inputs)[1] # dynamic
    with tf.variable_scope(scope, reuse=tf.AUTO_REUSE):
        # position indices
        position_ind = tf.tile(tf.expand_dims(tf.range(headskip,headskip+T), 0), [N, 1]) # (N, T)
        # First part of the PE function: sin and cos argument
        position_enc = np.array([
            [pos / np.power(10000, (i-i%2)/E) for i in range(E)]
            for pos in range(maxlen+headskip)])
...
def train_attention(config):
    ...
    # add the sentence length influence to it
    ...
    voice_O = tf.matmul(voice_A,voice_V) # shape: (batch, max_length, 200)
    # mask again for later fcn
    voice_O = mask(voice_O, seq_len, mode='mul', max_len=config.max_sent_num) # shape: (batch, max_sent_num, 200)

    O = tf.concat([voice_O, voice_V], axis = -1) # [batch, max_sent_num, 200]

    with tf.variable_scope("sent_self_attention"):
        # add the position embedding and them add the transformer
        sent_pos_embed = positional_encoding(inputs=O,maxlen=200,headskip=0,scope='sent_pos_embed') # [batch,max_sent_num,200]
        O = O + sent_pos_embed
        sent_Q = tf.layers.dense(inputs=O, units=32, activation=None, use_bias=False,
                                    kernel_initializer=tf.truncated_normal_initializer(), name = 'sentat_wq') # [batch,max_sent_num,32]
        sent_K = tf.layers.dense(inputs=O, units=32, activation=None, use_bias=False,
                                    kernel_initializer=tf.truncated_normal_initializer(), name = 'sentat_wk') # [batch,max_sent_num,32]
        sent_V = O # [batch,max_sent_num,200]
        sent_A = tf.matmul(sent_Q, sent_K, transpose_b=True) # [batch,max_sent_num,max_sent_num]
        sent_A = tf.transpose(sent_A, [0,2,1])
        sent_A = mask(sent_A, seq_len, mode='add', max_len=config.max_sent_num)
        sent_A = tf.transpose(sent_A, [0,2,1])
        sent_A = tf.nn.softmax(sent_A) # [batch, max_sent_num, max_sent_num]
        sent_O = tf.matmul(sent_A,sent_V) # [batch, max_sent_num, 200]
        sent_O += sent_V # residual added
...
if __name__ == "__main__":
    ...
    config.model_path = '/workspace/HangLi/ocean_talk/Voice_Attention/model_w2vin_dahai/w2vmeantrain_rcon_symqch_10_osa_balan_2_ndcay/'
