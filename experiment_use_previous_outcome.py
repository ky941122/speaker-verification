...
# @Date   : 2019-07-26
# @Author : KangYu
# @File   : experiment_use_previous_outcome.py
...
def train_attention(config):
    ...
    O = tf.concat([voice_O, word_V], axis=-1)    # shape: (batch, max_sent_num, 400)
    ...
    previous_O = tf.placeholder(shape=[config.batch_size, config.max_sent_num, 400],
                                    dtype=tf.float32, name="previous_O")

    rnn_inputs = tf.concat([previous_O, O], axis=1)

    rnn_out_2, _ = tf.nn.bidirectional_dynamic_rnn(fw_cell_2, bw_cell_2, rnn_inputs, sequence_length=seq_len,
                                                 dtype=tf.float32)
    rnn_out_2 = tf.concat(rnn_out_2, axis=-1)

    rnn_out_2 = rnn_out_2[:, config.max_sent_num:, :]
    ...
    with tf.Session(config=gpu_config) as sess:
        ...
        O_from_last_step_train = np.zeros([config.batch_size, config.max_sent_num, 400], dtype=np.float32)
        O_from_last_step_test = np.zeros([config.batch_size, config.max_sent_num, 400], dtype=np.float32)
        O_from_last_step_dev = np.zeros([config.batch_size, config.max_sent_num, 400], dtype=np.float32)
        for iter in range(config.iteration):
            ...
            _, loss_cur, train_summary, acc_cur, base_acc_cur, train_O_cur = sess.run(
                [train_op, loss, merge_summary, accuracy, base_acc, O],
                feed_dict={voice_embed: train_voice_embed, word_idx: train_word_idx, sent_len: train_sent_len,
                           ...
                           previous_O: O_from_last_step_train})

            test_loss_cur, test_summary, test_acc_cur, test_base_acc_cur, test_O_cur = sess.run(
                [loss, merge_summary, accuracy, base_acc, O],
                feed_dict={voice_embed: test_voice_embed, word_idx: test_word_idx, sent_len: test_sent_len,
                           ...
                           previous_O: O_from_last_step_test})

            test_loss_cur_2, test_summary_2, test_acc_cur_2, test_base_acc_cur_2, dev_O_cur = sess.run(
                [loss, merge_summary, accuracy, base_acc, O],
                feed_dict={voice_embed: test_voice_embed_2, word_idx: test_word_idx_2, sent_len: test_sent_len_2,
                           ...
                           previous_O:O_from_last_step_dev})
            ...
            O_from_last_step_train = train_O_cur
            O_from_last_step_test = test_O_cur
            O_from_last_step_dev = dev_O_cur
