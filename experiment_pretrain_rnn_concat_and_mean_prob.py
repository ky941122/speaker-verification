...
# @Date   : 2019-08-07
# @Author : KangYu
# @File   : experiment_pretrain_rnn_concat_and_mean_prob.py
...
def train_attention(config):
    ...
    ###### equal prob for the same label sents ########

    temp_label = tf.expand_dims(label, axis=-1)
    temp_label = tf.matmul(1 - temp_label, temp_label, transpose_b=True) + tf.matmul(temp_label, 1 - temp_label,
                                                                                     transpose_b=True)
    mask_1 = tf.cast(tf.equal(temp_label, 0), tf.float32)

    mask_2 = tf.cast(tf.sequence_mask(seq_len, config.max_sent_num), tf.float32)

    mask_2 = tf.expand_dims(mask_2, axis=-1)  # [batch, max_length, 1]
    mask_2 = tf.matmul(mask_2, mask_2,
                       transpose_b=True)  # [batch, max_length, max_length]
    mask = mask_1 * mask_2

    word_embed2 = tf.matmul(mask, word_embed)

    class_cnt = tf.reduce_sum(mask, axis=-1)
...
if __name__ == "__main__":
    ...
    config.model_name = "pretrain_e-2_100_concat_equal_prob"
