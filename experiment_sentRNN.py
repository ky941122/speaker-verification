...
def train_attention(config):
    ...
    rnn_out, _ = tf.nn.bidirectional_dynamic_rnn(fw_cell, bw_cell, dense1, sequence_length=seq_len,
                                                 dtype=tf.float32)
    rnn_out = tf.concat(rnn_out, axis=-1)

    dense2 = tf.layers.dense(inputs=rnn_out, units=64, activation=tf.nn.relu,
                             kernel_initializer=tf.truncated_normal_initializer(),
