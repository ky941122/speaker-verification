...
def parse_helper(example_proto):
    ...
    sent_key_idx = tf.decode_raw(parsed_example['sent_key_idx'], tf.int32)
    sent_key_idx = tf.reshape(sent_key_idx, parsed_example['sent_word_shape'])

    length = tf.cast(parsed_example['length'], tf.int32)
    return voice_embed, sent_word_idx, sent_tag_idx, sent_key_idx, sent_word_num, sent_label, length
...
def train_attention(config):
    ...
    key_idx = tf.placeholder(shape=[config.batch_size, config.max_sent_num, config.max_sent_len],
                              dtype=tf.int32)  # shape: (batch, max_sent_num, max_sent_len)
    ...
    t2v_embedding = tf.get_variable("tags_embedding", shape=[config.tags_size, config.w2v_dim], dtype=tf.float32)
    k2v_embedding = tf.get_variable("key_embedding", shape=[config.key_size, config.w2v_dim], dtype=tf.float32)
    ...
    key_embed = tf.nn.embedding_lookup(k2v_embedding, key_idx)
    word_embed = word_embed + tag_embed + key_embed
    ...
    # voice_V = tf.layers.dense(inputs=voice_embed, units=32, activation=None, use_bias=False,
    #                           name='attention_w', reuse=True)
    ...
    O = tf.concat([voice_O, word_V, voice_K], axis=-1)
    ...
    train_root = '/share/kangyu/speaker_verification/new_data/dahai/train'
    ...
    parsed_train = parsed_train.apply(
        tf.contrib.data.padded_batch_and_drop_remainder(
            batch_size=config.batch_size, padded_shapes=(
                ...
                [config.max_sent_num, config.max_sent_len], [config.max_sent_num, config.max_sent_len],
    ...
    test_zhikang_root = '/share/kangyu/speaker_verification/new_data/zhikang/test'
    ...
    test_dahai_root = '/share/kangyu/speaker_verification/new_data/dahai/test'
    ...
    with tf.Session(config=gpu_config) as sess:
        ...
        for iter in range(config.iteration):
            ...
            train_voice_embed, train_word_idx, train_tag_idx, train_key_idx, train_sent_len, train_label, train_seq_len = sess.run(train_next)
            test_voice_embed, test_word_idx, test_tag_idx, test_key_idx, test_sent_len, test_label, test_seq_len = sess.run(test_zhikang_next)
            test_voice_embed_2, test_word_idx_2, test_tag_idx_2, test_key_idx_2, test_sent_len_2, test_label_2, test_seq_len_2 = sess.run(
                test_dahai_next)

            _, loss_cur, train_summary, acc_cur, base_acc_cur = sess.run(
                [train_op, loss, merge_summary, accuracy, base_acc],
                feed_dict={voice_embed: train_voice_embed, word_idx: train_word_idx, tag_idx: train_tag_idx, key_idx: train_key_idx,
            ...
            test_loss_cur, test_summary, test_acc_cur, test_base_acc_cur = sess.run(
                ...
                feed_dict={voice_embed: test_voice_embed, word_idx: test_word_idx, tag_idx: test_tag_idx, key_idx: test_key_idx,
            ...
            test_loss_cur_2, test_summary_2, test_acc_cur_2, test_base_acc_cur_2 = sess.run(
                [loss, merge_summary, accuracy, base_acc],
                feed_dict={voice_embed: test_voice_embed_2, word_idx: test_word_idx_2, tag_idx: test_tag_idx_2, key_idx: test_key_idx_2,
