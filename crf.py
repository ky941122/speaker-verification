...
def crf_sequence_score(inputs, tag_indices, sequence_lengths,
  ...
  def _multi_seq_fn():
    ...
    return sequence_scores  #[batch, max_len]
...
def crf_log_norm(inputs, sequence_lengths, transition_params):
  ...
  def _multi_seq_fn():
    ...
    log_norm = math_ops.log(math_ops.exp(alphas))   #[batch, maxlen]
    # Mask `log_norm` of the sequences with length <= zero.
    # log_norm = array_ops.where(math_ops.less_equal(sequence_lengths, 0),
    #                            array_ops.zeros_like(log_norm),
    #                            log_norm)
...
def crf_log_likelihood(inputs,
  ...
  sequence_scores = crf_sequence_score(inputs, tag_indices, sequence_lengths,
                                       transition_params)
  print("scores", sequence_scores.shape)
  log_norm = crf_log_norm(inputs, sequence_lengths, transition_params)
  print("norm", log_norm.shape)
...
class CrfForwardRnnCell(rnn_cell.RNNCell):
  ...
  def __init__(self, transition_params):
    ...
    self._num_tags = transition_params.shape[0].value
